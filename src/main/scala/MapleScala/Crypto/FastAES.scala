...
class FastAES {
  ...
  /**
   * Transforms a block using AES
   * @param buffer Buffer to write in
   */
  def TransformBlock(buffer: Array[Int]): Unit = {
    var C0 = buffer(0)
    var C1 = buffer(1)
    var C2 = buffer(2)
    var C3 = buffer(3)
  ...
  private[Crypto] def generateWorkingKey: Array[Array[Int]] = {
    val key = generateKey
    val keyLength = 8
    val workingKey = new Array[Array[Int]](rounds + 1)

    for (i <- 0 to rounds)
      workingKey(i) = new Array[Int](4)

    for (i <- 0 until keyLength)
      workingKey(i >> 2)(i & 3) = ToInt(key, i * 4)
    ...
    for (i <- keyLength until k) {
      ...
      workingKey(i >>> 2)(i & 3) = workingKey((i - keyLength) >>> 2)((i - keyLength) & 3) ^ temp
    }

    workingKey
  }

  private def generateKey: Array[Byte] = {
    val reverse = ByteBuffer
      .allocate(8)
      .order(ByteOrder.BIG_ENDIAN)
      .putLong(CWvsKey)
      .array()

    val key = new Array[Byte](8 * 4)
    for (i <- 0 until 8)
      key(i * 4) = reverse(i)
    key
  }

  private[Crypto] def ToInt(buffer: Array[Byte], offset: Int): Int = (buffer(offset) & 0xFF) | (buffer(offset + 1) & 0xFF) << 8 | (buffer(offset + 2) & 0xFF) << 16 | (buffer(offset + 3) & 0xFF) << 24
...
object FastAES {
  ...
  private final val Table0: Array[Int] = Array(
  ...
  private final val Table1: Array[Int] = Array(
  ...
  private final val Table2: Array[Int] = Array(
  ...
  private final val Table3: Array[Int] = Array(
